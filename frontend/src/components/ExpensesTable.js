...
const ExpensesTable = () => {
  ...
  const fetchExpenses = useCallback(async (page, limit) => {
    ...
    try {
      ...
      const fetchedTotal = response.data.data.total;
      const fetchedPage = response.data.data.page;
  ...
  const handleRowDelete = async (id) => {
    ...
    // Store original state for potential rollbackƒ
    const originalExpenses = [...expenses];
    const originalTotal = totalRows;
              ...
              {expenses.map((expense, index) => {
                ...
                return (
                  <div
                    ...
                    className={`relative bg-white border border-slate-200 rounded-xl p-5 transition-all duration-200 group cursor-pointer overflow-hidden animate-[fadeIn_0.4s] ${isSelected ? 'ring-2 ring-indigo-400 border-indigo-400' : ''
                  ...
                  >
                    ...
                    <div className="flex flex-col gap-2 pl-10 pr-12">
                      ...
                      <div className="flex items-center justify-between mt-2">
                        <span className={`inline-flex items-center px-3 py-1 rounded-full text-xs font-semibold ${expense.amount > 0
                            ? 'bg-emerald-100 text-emerald-700 border border-emerald-200'
                            : 'bg-amber-100 text-amber-700 border border-amber-200'
                          }`}>
                          <span className={`w-2 h-2 rounded-full mr-2 ${expense.amount > 0 ? 'bg-emerald-500' : 'bg-amber-500'
                            }`}></span>
